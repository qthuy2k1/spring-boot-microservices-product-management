    ...
    public ReportModel getOrderReport(String startDate, String endDate) {
        Query query = entityManager.createNativeQuery("""
                ...
                SELECT
                    ...
                    COALESCE((SELECT COUNT(*) FROM FilteredOrders WHERE status = 0), 0) AS pending,
                    COALESCE((SELECT COUNT(*) FROM FilteredOrders WHERE status = 1), 0) AS processing,
                    COALESCE((SELECT COUNT(*) FROM FilteredOrders WHERE status = 2), 0) AS shipped,
                    COALESCE((SELECT COUNT(*) FROM FilteredOrders WHERE status = 3), 0) AS delivered,
                    COALESCE((SELECT COUNT(*) FROM FilteredOrders WHERE status = 4), 0) AS canceled
                FROM FilteredOrders
                """);
